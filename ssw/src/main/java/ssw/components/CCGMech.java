...
/**
 *
 * CCGMech - Class that generates a Battletech CCG card based upon data provided
 * from Mech class. For now, use dlgCCGMech to display the relevant information.
 *
 * CCGMech handles the conversion of a 'Mech from the tabletop rules into a card
 * that can be used in the Battletech CCG game.
 *
 * The goal of this class is to reproduce the stats on the cards for those
 * 'Mechs already included in the game. Other 'Mechs should be generated in a
 * manner consistent with the canon cards in an effort to keep play balanced.
 *
 * The current conversion process is a little long to be detailed here, but they
 * are based upon Gauss Bears Inofficial Conversion Rules.
 *
 * There are still a few special abilities to generate, 'Mech cost to calculate,
 * and a whole bunch of fiddling with equations. In addition, the class
 * currently does not check if a canon card already exists.
 *
 * @author Kevin
 */
public class CCGMech {
    ...
    private int Mass = 0,
            ...
            AttackValue = 0, ArmourValue = 0, StructureValue = 0, MissileDamage = 0,
            // OverheatDamage = 0,
            AlphaStrikeDamage = 0, LeakDamage = 0;
    ...
    /**
     * This constructor should build the entire card. Possibly an entire Mech object
     * can be passed in here, although this system works for now.
     *
     * Eventually, this constructor should check to see if there exists a canon CCG
     * card, in which case those stats should be used. Otherwise we can generate a
     * (hopefully) balanced card using the conversion process.
     *
     * @param name     The 'Mech's name without variant info. Normally
     *                 aMech.GetName().
     * @param fullname The full name of the unit (appears on top of card. Normally
     *                 aMech.GetFullName();
     */
    ...
    /**
     * Sets the movement rate for the card based upon the 'Mech's walking speed.
     *
     * [0-2] Slow, Slowpoke ability. [3] Slow [4-6] Moderate [7-8] Fast [9+] Fast,
     * Dasher ability.
     *
     * TODO: Possibility of MASC or Supercharger ability. Something like: MASC
     * (Place a MASC token on this card. The 'Mech takes 1 Damage for every MASC
     * token on the card, but may operate as if it was one speed level faster.)
     *
    ...
    /**
     * IF the mech has a single jump jet, it gets the jump ability.
     *
     * TODO: Possiblity of an IJJ ability. Something like: Improved Jump (-1 Attack,
     * +1 Initiative), 'Mech operates as if it was one speed level faster.
     *
     * @param jump
     */
    ...
    /**
     * The 'Mech's mass is used when assigning structure values. Heavier mechs tend
     * to have more armour than lighter ones. You should set the mass before you
     ...
     * mess of an if statement. Each piece of equipment from the loadout is checked
     * to see if it is a weapon. If it is, check to see if it is rear-mounted (in
     * which case it is ignored) or an MG (in which case it gives the card the
     * AntiPersonnel ability.
     *
     * From there, each main class of weapons is checked. Basic algorithm is that
     * weapon damage is divided by a range factor (long range weapons have a lower
     * range factor) to produce an Attack value for that weapon.
     *
     * Missile weapons are split between LRMs and all others. LRMs increase a Mech's
     * missile ability, while MRMs and SRMs increase the attack value. (unlike in
     * the conversion rules, SRMs do not currently grant alpha strike, although I
     * think that maybe MRMs should). Artemis IV is accounted for, but NARC and
     * Apollo currently aren't.
     *
     * TODO: NARC and MRMs to Alpha Strike.
     *
     * Next is ballistic weapons. Ac-20s of all types give +2 Alpha strike and count
     * half damage and heat towards Attack value. Ac-10s and Heavy Gauss Rifles
     * count +1 to Alpha Strike and half damage and heat as well. Gauss and Heavy
     * Gauss also count +1 to leak damage for long range fire. Remaining ballistic
     * weapons add to Attack Value only.
     *
     * TODO: Light Gauss rifle.
     *
     * Finally Energy weapons (and all others). 3 or more PPCs or ERLL contribute a
     * +1 to leak damage, while 1 ER PPC does the same. All Attack values from
     * energy weapons are multiplied by 1.3 as well. Unlike the conversion rules,
     * there is no range factor reduction for IS Pulse Weapons, although there
     * probably should be.
     *
     * The overheat situation is all borked up, I have no idea how to properly
     * implement it. The conversion rules are all fuzzy and happy about it, but it's
     * basically fudge work.
     *
     ...
     * damage under the (faulty) assumption that all weapons have identical heat to
     * damage ratios.
     *
     *
     * @param CurrentLoadout The ArrayList from a Mech class with weapons and
     *                       equipment.
     * @param MovementHeat   The maximum heat the 'Mech generates by moving (Usually
     *                       2 unless it jumps).
    ...
    public void setAttackValue(ArrayList<abPlaceable> CurrentLoadout, int MovementHeat, int TotalHeatDis) {
        ...
        if (CurrentLoadout.size() > 0) {
            for (int i = 0; i < CurrentLoadout.size(); i++) {
                if (CurrentLoadout.get(i) instanceof ifWeapon) {
                    ...
                    // Although these are assigned, they are not used anywhere.
                    ...
                    if (((abPlaceable) currentWeapon).IsMountedRear()) {
                        ...
                        SpecialAbilities = SpecialAbilities + AblPA1 + Name + AblPA2 + MovementRate + AblPA3 + Name
                                + AblPA4 + MovementRate + AblPA5 + "\n";
                    ...
                    // System.out.println(currentWeapon.GetName() + " " + currentWeapon.GetType() +
                    // " " + currentWeapon.GetDamageMedium());
        ...
        /*
         * Both this batch and the batch after the sort appear to be Debuging statements
         * to confirm sorting. First was already commented out, section had the output
         * commented. Just made both uniform for styling and remove an effective no-op.
         */
        ...
        // for(int c = 0; c < weaponList.size(); c++) {
        // WeaponData wep = weaponList.get(c);
        // // System.out.println("AttackValue: " + wep.Damage + " HeatValue: " +
        // wep.Heat);
        // }
        ...
        // System.out.println("Attack value: " + attackValue + " MissileValue: " +
        // missileValue);
        if (MissileDamage > 0) {
